...
import { Factory, Search, History, CheckCircle } from 'lucide-react';
...
export default function ProcessorDashboard() {
  ...
  const tabs = [
    { id: 'record', label: 'Record Event', icon: Factory },
    ...
    <div className="min-h-screen bg-gray-50 pb-16">
      <div className="bg-white border-b border-gray-100 px-4 py-6">
        <div className="max-w-4xl mx-auto flex items-center gap-3">
          ...
          <span className="ml-auto px-3 py-1 bg-blue-100 text-blue-800 text-xs font-bold rounded-full">PROCESSOR</span>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
          ...
          {tabs.map(({ id, label, icon: Icon }) => (
            <button key={id} onClick={() => setTab(id)}
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-colors ${
                tab === id ? 'bg-blue-600 text-white shadow-sm' : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-100'
              }`}>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              <h2 className="text-base font-bold text-gray-900 mb-4 flex items-center gap-2">
                <Search className="w-4 h-4 text-blue-600" /> Find Product to Process
              </h2>
            ...
            {selected && !done && (
              <div className="bg-white rounded-2xl border border-blue-100 shadow-sm p-6">
                <h2 className="text-base font-bold text-gray-900 mb-4 flex items-center gap-2">
                  <Factory className="w-4 h-4 text-blue-600" /> Record Processing Event
                </h2>
                <RecordEventPanel
                  product={selected}
            ...
            {done && (
              <div className="bg-green-50 border border-green-200 rounded-xl p-4 flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-green-800">Event recorded successfully!</p>
        ...
        {tab === 'history' && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <h2 className="text-base font-bold text-gray-900 mb-4">My Recorded Events ({history.length})</h2>
            {loading ? (
                ...
                {history.map((h, i) => (
                  <div key={i} className="flex items-center justify-between py-3 border-b border-gray-50 last:border-0">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">{EVENT_NAMES[h.eventType] || h.eventType}</p>
                    </div>
